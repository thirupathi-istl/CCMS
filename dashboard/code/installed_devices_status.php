...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (!$conn_db_all) {
		...
		if ($device_status == "ACTIVE_DEVICES") {
			...
			WHERE active_device='1' AND installed_status = '1'AND device_id IN ($placeholders)";
		} 
		elseif ($device_status == "POOR_NW_DEVICES") {
			...
			WHERE poor_network='1' AND installed_status = '1'AND device_id IN ($placeholders)";
		} 
		elseif ($device_status == "POWER_FAIL_DEVICES") {
			...
			WHERE power_failure='1' AND installed_status = '1'AND device_id IN ($placeholders)";
		} 
		elseif ($device_status == "FAULTY_DEVICES") {
			...
			WHERE faulty='1' AND installed_status = '1'AND device_id IN ($placeholders)";
		} 
