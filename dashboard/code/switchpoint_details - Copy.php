...
//////////////////////////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		if(mysqli_query($conn, $sql_lights))
		{
			$result = mysqli_query($conn, $sql_lights);
		...
		if(mysqli_query($conn, $sql_installed_status))
		{
			$result_installed_status = mysqli_query($conn, $sql_installed_status);
			if(mysqli_num_rows($result_installed_status)>0)
			{
				while($r_installed_status = mysqli_fetch_assoc( $result_installed_status ))
				{
		...
		if(mysqli_query($conn, $sql_active_status))
		{
			$result = mysqli_query($conn, $sql_active_status);
			if(mysqli_num_rows($result)>0)
			{
				while($rl = mysqli_fetch_assoc( $result ))
				{
	...
	$return_response= array("TOTAL_UNITS"=>$total_switch_point, "SWITCH_POINTS"=>$installed_switch_points, "UNISTALLED_UNITS"=>$uninstalled_devices, "ACTIVE_SWITCH"=>$active_switch_points, "POOR_NW"=>$poor_network, "POWER_FAILURE"=>$power_failure_count, "FAULTY_SWITCH"=>$inactive_switch_points, "TOTAL_LIGHTS"=>$installed_lights, "ON_LIGHTS"=>$total_load, "OFF_LIGHT"=>$off_percentage, "FAULTY_LIGHT"=>"0", "INSTALLED_LOAD"=>$installed_load, "ACTIVE_LOAD"=>$kw, "KWH"=>round($kwh, 2), "KVAH"=>"0.12", "SAVED_UNITS"=>round($unitsSaved, 2), "SAVED_AMOUNT"=>round($amountSaved,2), "SAVED_CO2"=>round($amountCo2, 2),   "ON_UNITS"=>$auto_system_on, "MANUAL_ON"=>$manual_on , "OFF"=>$off);
...
else
{
	...
	$return_response= array("SWITCH_POINTS"=>"--", "ACTIVE_SWITCH"=>"--", "FAULTY_SWITCH"=>"--", "TOTAL_LIGHTS"=>"--", "ON_LIGHTS"=>"--", "OFF_LIGHT"=>"--", "FAULTY_LIGHT"=>"--", "INSTALLED_LOAD"=>"--", "ACTIVE_LOAD"=>"--", "KWH"=>"--", "KVAH"=>"--", "SAVED_UNITS"=>"--", "SAVED_AMOUNT"=>"--", "SAVED_CO2"=>"--", "POOR_NW"=>$poor_network, "TOTAL_UNITS"=>$total_switch_point, "UNISTALLED_UNITS"=>$uninstalled_devices, "POWER_FAILURE"=>$power_failure_count, "ON_UNITS"=>$auto_system_on, "MANUAL_ON"=>$manual_on , "OFF"=>$off);
}
