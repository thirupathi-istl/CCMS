<!-- Active Status Column modal -->
<div class="modal fade" id="openview">
    <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="lightsModalLabel">Status</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                            <div class="col-12 rounded mt-3 p-0">
                                <div class="row">
                                    <div class="col-xl-3 col-6">
                                        <div class="card text-center shadow">
                                        <div class="card-body m-0 p-0">
                                            <p class="card-text fw-semibold m-0 py-1"><i class="bi bi-lightbulb-fill h4"></i> Total Lights</p>                                            
                                            <h3 class="card-title py-2">70</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-3 col-6">
                                        <div class="card text-center shadow">
                                        <div class="card-body m-0 p-0">
                                            <p class="card-text fw-semibold m-0 py-1 "><i class="bi bi-power h4"></i>ON</p>
                                            <h3 class="card-title py-2 ">80%</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-3 col-6 mt-3 mt-xl-0" >
                                        <div class="card text-center shadow">
                                        <div class="card-body m-0 p-0">
                                        <p class="card-text fw-semibold m-0 py-1 "><i class="bi bi-power h4"></i>OFF</p>
                                        <h3 class="card-title py-2 ">20%</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-3 col-6 mt-3 mt-xl-0">
                                        <div class="card text-center shadow" >
                                        <div class="card-body m-0 p-0">
                                            <p class="card-text fw-semibold m-0 py-1 "><i class="bi bi-lightning-fill h4"></i> Voltage</p>
                                            <h3 class="card-title py-2">60</h3>
                                        </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                             <div class="mt-4">
                                <h5>Voltage</h5>
                            </div>
                            <div class="col-12 rounded  p-0">
                                <div class="row">
                                    <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4 ">
                                        <div class="card  shadow bg-danger mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">R</h5>                                            
                                            <h3 class="card-title py-2 text-center">15</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4 col-lg-4  col-md-4 col-sm-4 col-4">
                                        <div class="card  shadow bg-warning mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">Y</h5>                                            
                                            <h3 class="card-title py-2 text-center">14</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4  col-lg-4 col-md-4 col-sm-4 col-4" >
                                        <div class="card  shadow bg-primary mt-2">
                                        <div class="card-body m-0 p-0">
                                        <h5 class="card-text fw-semibold m-0 py-1 px-1">B</h5>                                            
                                        <h3 class="card-title py-2 text-center">12</h3>
                                        </div>
                                        </div>
                                    </div>
                                  
                                </div>
                            </div>

                            <div class="mt-4">
                                <h5>Current</h5>
                            </div>
                            <div class="col-12 rounded  p-0">
                                <div class="row">
                                    <div class="col-xl-4  col-lg-4 col-md-4 col-sm-4 col-4">
                                        <div class="card  shadow bg-danger  mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">R</h5>                                            
                                            <h3 class="card-title py-2 text-center">15</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4  col-lg-4 col-md-4 col-sm-4 col-4">
                                        <div class="card  shadow bg-warning  mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">Y</h5>                                          
                                            <h3 class="card-title py-2 text-center">14</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4  col-lg-4 col-md-4 col-sm-4 col-4" >
                                        <div class="card  shadow bg-primary  mt-2">
                                        <div class="card-body m-0 p-0">
                                        <h5 class="card-text fw-semibold m-0 py-1 px-1">B</h5>
                                        <h3 class="card-title py-2 text-center">12</h3>
                                        </div>
                                        </div>
                                    </div>
                                  
                                </div>
                            </div>

                             <div class="mt-4">
                                <h5>Power</h5>
                            </div>
                            <div class="col-12 rounded  p-0">
                                <div class="row">
                                    <div class="col-xl-4 col-lg-4">
                                        <div class="card  shadow bg-danger  mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">R</h5>                                           
                                            <h3 class="card-title py-2 text-center">15</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4 col-lg-4">
                                        <div class="card  shadow bg-warning  mt-2">
                                        <div class="card-body m-0 p-0">
                                            <h5 class="card-text fw-semibold m-0 py-1 px-1">Y</h5>                                            
                                            <h3 class="card-title py-2 text-center">14</h3>
                                        </div>
                                        </div>
                                    </div>
                                    <div class="col-xl-4 col-lg-4" >
                                        <div class="card  shadow bg-primary  mt-2">
                                        <div class="card-body m-0 p-0">
                                        <h5 class="card-text fw-semibold m-0 py-1 px-1">B</h5>
                                        <h3 class="card-title py-2 text-center">12</h3>
                                        </div>
                                        </div>
                                    </div>
                                  
                                </div>
                            </div>                          
                </div>
            <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                </div>
            </div>
    </div>

</div>
