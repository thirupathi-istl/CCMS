...
function addDevice() {
    ...
    newRow.innerHTML = '\
        ...
        <td>' + capacity + '</td>\
        <td class="col-1">' + getCurrentTime() + ' '+ getCurrentDate() + '</td>\
        <td>Off</td>\
        <td>Manual</td>\
        <td>Active</td>\
        <td><a href="https://www.google.com/maps?q=17.467754,%2078.603072" target="_blank">Location</a></td>\
        <td><button class="btn btn-info btn-sm" onclick="openLightsModal(this)">0</button></td>\
...
function openLightsModal(element) {
    ...
    lightsData.forEach(light => {
        ...
        row.innerHTML = '\
        <td>' + light.brandName + '</td>\
        <td>' + light.wattage + '</td>\
        <td>' + light.lights + '</td>\
        <td>' + (light.lights * light.wattage) + '</td>\
        <td><i class="bi bi-trash-fill text-danger" onclick="deleteLightRow(this)"></i></td>\
        ';
