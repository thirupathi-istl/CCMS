...
if (isset($_POST['REQ']))
{
   ...
   //$data="MCMS_4;ACK;SERIALNO;";            //DEVID, ECLR,   WIFI, SERIALNO
	if ($data != "")
	{
		...
		{
			/*if($array_data[0]=="CCMS_11"||$array_data[0]=="CCMS_13")
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////
    function mainfunction($data, $conn)
    {
    	...
    	$sql = "SELECT *from (SELECT * FROM device_settings WHERE setting_type IN ('ONOFF', 'VOLTAGE', 'CURRENT', 'DRY_RUN', 'FRAME_TIME', 'HYSTERESIS', 'CALIB_VALUES', 'SOFTWARE', 'RESET', 'ID_CHANGE', 'ENERGY_RESET', 'SERIAL_ID', 'WIFI_CREDENTIALS') ORDER BY setting_type DESC) a UNION ALL (SELECT * FROM device_settings WHERE setting_type='DEFAULT' LIMIT 1)";
    ...
    function voltage_current_alerts($deviceid, $type, $phase, $normal_phases, $conn, $status, $v1, $v2, $v3, $c1, $c2, $c3, $date)
    {
    	...
    	switch ($type)
    	{
    		...
    		if ($status == 1)
    		{
    			...
    			{
    				return mainfunction($deviceid . ";", $conn);
    			}
    		}
    		else
    		{
