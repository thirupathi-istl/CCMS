    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") 
    {
        ...
        // Create a new database connection
            ...
            switch (trim($parameter)) {
                ...
                $sql_mode = "INSERT INTO `limits_pf` (`device_id`, `pf`, `date_time`, `user_mobile`, `email`, `name`, `role`) 
                VALUES (?, ?, current_timestamp(), ?, ?, ?, ?)";

                $stmt = mysqli_prepare($conn_db, $sql_mode);
                if ($stmt) {
                    mysqli_stmt_bind_param($stmt, 'ssssss', $device_id, $updated_value, $mobile_no, $user_email, $user_name, $role);
                    mysqli_stmt_execute($stmt);
                    mysqli_stmt_close($stmt);
                } else {
                ...
                $sql_mode = "INSERT INTO `unit_capacity` (`device_id`, `capacity`, `date_time`, `user_mobile`, `email`, `name`, `role`) 
                VALUES (?, ?, current_timestamp(), ?, ?, ?, ?)";
                ...
                $device_id=strtoupper($device_id);
                $sql_mode = "INSERT INTO `frame_time` (`device_id`, `frame_time`, `date_time`, `user_mobile`, `email`, `name`, `role`) 
                VALUES (?, ?, current_timestamp(), ?, ?, ?, ?)";
                ...
                $setting_sql = "INSERT INTO device_settings (`setting_type`, `setting_flag`) VALUES ('FRAME_TIME', '1') 
                ...
                $read_sql = "INSERT INTO device_settings (`setting_type`, `setting_flag`) VALUES ('READ_SETTINGS', '1') 
                ON DUPLICATE KEY UPDATE setting_flag='1'";
                ...
                $device_id=strtoupper($device_id);
                $sql_mode = "INSERT INTO `limits_ct_ratio` (`device_id`, `ct_ratio`, `date_time`, `user_mobile`, `email`, `name`, `role`) 
                VALUES (?, ?, current_timestamp(), ?, ?, ?, ?)";
